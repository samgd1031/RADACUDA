...
inline rectangle::rectangle(point3 orig, vec3 v1, vec3 v2, double dim1,
  ...
  if (dot(S1, v2) != 0.0) {  // dot product is zero if vectors are perpendicular
    vec3 parallel = (dot(S1, v2) / S1.length_squared()) *
    ...
    S2 = unit_vector(v2);  // normalize vector
  ...
  normal = cross(S1, S2);  // since S1 and S2 are already orthogonal unit
...
inline bool rectangle::hit(const ray& r, double t_min, double t_max,
  ...
  // rectangle
  vec3 v1 = S1 * d1;
  std::vector<bool> r_HO1_pass = {
      r_HO1
          .e[0]<0,
                r_HO1.e[1]<0, r_HO1.e[2]<0, r_HO1.e[0]> v1.e[0], r_HO1.e[1]> v1
                    .e[1],
                r_HO1.e[2]>
              v1.e[2]};
  ...
  // rectangle
  vec3 v2 = S2 * d2;
  std::vector<bool> r_HO2_pass = {
      r_HO2
          .e[0]<0,
                r_HO2.e[1]<0, r_HO2.e[2]<0, r_HO2.e[0]> v2.e[0], r_HO2.e[1]> v2
                    .e[1],
                r_HO2.e[2]>
              v2.e[2]};
...
inline std::string rectangle::gnuplot_repr() {
  std::string x_str = std::to_string(origin[0]) + " + " +
                      std::to_string((S1 * d1).e[0]) + "*u + " +
                      std::to_string((S2 * d2).e[0]) + "*v";
  std::string y_str = std::to_string(origin[1]) + " + " +
                      std::to_string((S1 * d1).e[1]) + "*u + " +
                      std::to_string((S2 * d2).e[1]) + "*v";
  std::string z_str = std::to_string(origin[2]) + " + " +
                      std::to_string((S1 * d1).e[2]) + "*u + " +
                      std::to_string((S2 * d2).e[2]) + "*v";
