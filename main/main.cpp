...
int main() {
  ...
  gp.sendLine("splot " + orig_rect.gnuplot_repr() +
              " with pm3d fs solid fc '#A00000FF'");
  gp.sendLine("replot " + tgt_rect.gnuplot_repr() +
              " with pm3d fs solid fc '#A0FF0000'");
  ...
  for (int ii = 0; ii < N_rays; ++ii) {
    ...
    if (ray_hits) {
      hit_count++;
      gp.sendLine("replot " + ray_list[ii].gnuplot_repr(0, 1) +
                  "linecolor 'green'");
  ...
  std::cout << "Estimated View Factor:\t" << std::fixed << view_factor << "("
            << hit_count << "/" << N_rays << ")\n";

  // stop timer
  auto tend = std::chrono::high_resolution_clock::now();
  std::chrono::duration<double, std::milli> ms_double = tend - tstart;
  std::cout << "Elapsed Time:\t" << std::fixed << ms_double.count() / 1000.0
            << " seconds\n";
